...
def get_storage_type():
    storage_dict = {1: "JSON", 2: "PostgreSQL", 3: "MySQL"}
    storage = None
    while storage is None:
        ...
        print("3. MySQL (Requires a database server)")
        storage = input("> ")
        try:
            storage = int(storage)
        except ValueError:
...
def basic_setup():
    ...
    storage_dict = {1: BackendType.JSON, 2: BackendType.POSTGRES, 3: BackendType.MYSQL}
